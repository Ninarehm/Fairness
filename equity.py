	...
	for h in range(10):
		...
		for i in range(10):
			...
			with tf.Session() as sess:
				...
				dataset =pd.Series(pred_test[int(-1*general_n_test):])
				X = pd.concat([X[int(-1*general_n_test):],X]).reset_index(drop=True)
				Y = pd.concat([dataset,Y]).reset_index(drop=True)
				X.drop(X.tail(int(1*general_n_test)).index,inplace=True)
				Y.drop(Y.tail(int(1*general_n_test)).index,inplace=True)
				n_train = n_train+(int(1*general_n_test))
				n_test = n_test-(int(1*general_n_test))
...
if __name__=="__main__":
	...
	overall_numbers = classifier(float(args.beta),args.loss_type)

	print("Mean: " + str(statistics.mean(overall_numbers[1])) )

	first_step_results = statistics.mean(overall_numbers[1])

	print("***************************Baseline Classifier***************************")

	classifier_numbers = classifier(0.0,'equality')
	second_step_results = statistics.mean(classifier_numbers[1])

	print('****************Fairness Gain is****************')
	print(abs(first_step_results-second_step_results) *100)
